...
// Updated: Ticker links to internal news page [11.06.2026]
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full ${style.bg} ${style.text} text-xs font-medium border ${style.border} shadow-lg ${style.glow} transition-all duration-300 hover:scale-105`}>
      <span className="relative flex h-2 w-2">
        {status === 'in-development' && <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-yellow-400 opacity-75" />}
...
// Combined JSON-LD with @graph (WebSite + Organization + SearchAction)
const JsonLd = () => (
  <script
        ...
        "@graph": [
          {
            ...
            "potentialAction": {
              "@type": "SearchAction",
              "target": "https://minimaverse.com/?q={search_term_string}",
...
const LatestUpdatesStrip = () => {
  ...
  const content = (
    ...
    >
      ...
      <span className="mr-2">Maximize rewards programme concluded after distributing over 50 million $MINIMA. Existing contracts continue operating until maturity</span>
    </Link>
  );
    ...
    <div className="relative max-w-4xl mx-auto px-4 sm:px-0">
        ...
        <div className="relative flex flex-col sm:flex-row sm:justify-between sm:items-start gap-6">
          ...
          <div className="flex-1 min-w-0 pl-2 pr-2 sm:pl-4 sm:pr-4">
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-4 relative inline-block">
              <span className="bg-gradient-to-r from-purple-200 via-blue-200 to-white bg-clip-text text-transparent">Minimaverse</span>
            ...
            <p className="text-gray-400 text-base sm:text-lg mb-3 leading-relaxed">Independent knowledge base for the Minima protocol.</p>
            <p className="text-gray-400 text-sm mb-2">Verified documentation, node guides, and ecosystem resources — no hype, no noise.</p>
            <p className="text-gray-400/65 text-xs mt-2 flex flex-wrap gap-x-3 gap-y-1">
              <span>Learn about Minima.</span><span className="hidden sm:inline">•</span><span>Set up your own node.</span><span className="hidden sm:inline">•</span><span>Explore the ecosystem.</span>
            </p>
            <div className="flex flex-col sm:flex-row sm:items-center sm:gap-4 px-2 sm:px-0 mt-4">
              <Link href="/protocol" className="group inline-flex items-center justify-center px-6 sm:px-8 py-3 bg-gradient-to-r from-purple-600 to-blue-600 text-white font-semibold rounded-xl hover:from-purple-500 hover:to-blue-500 transition-all duration-300 min-w-[200px] sm:min-w-[220px] shadow-lg shadow-purple-500/25 hover:shadow-purple-500/40 hover:scale-105 hover:-translate-y-0.5 text-sm sm:text-base">
                How Minima Works<svg className="w-4 h-4 sm:w-5 sm:h-5 ml-2 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
              </Link>
              <Link href="/nodes" className="group inline-flex items-center justify-center px-6 sm:px-8 py-3 mt-3 sm:mt-0 border-2 border-gray-600 text-gray-300 font-semibold rounded-xl hover:border-purple-500/50 hover:text-white hover:bg-purple-500/10 transition-all duration-300 min-w-[200px] sm:min-w-[220px] hover:shadow-lg hover:shadow-purple-500/20 hover:scale-105 hover:-translate-y-0.5 text-sm sm:text-base">
                Run a Node<svg className="w-4 h-4 sm:w-5 sm:h-5 ml-2 group-hover:rotate-45 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 10l7-7m0 0l7 7m-7-7v18" /></svg>
              </Link>
              <Link href="/about" className="group inline-flex items-center gap-1 text-gray-400 hover:text-purple-400 text-sm underline decoration-gray-600 hover:decoration-purple-500/60 underline-offset-4 transition-colors duration-200 ease-out mt-3 sm:mt-0 sm:ml-2 cursor-pointer">
                Start Here <span className="transform group-hover:translate-x-1 transition-transform duration-200 ease-out">→</span>
              </Link>
            </div>
          </div>
        </div>
      ...
      <div className="relative bg-gray-800/40 border border-gray-700/40 rounded-2xl p-4 sm:p-6 mb-8 transition-all duration-300 hover:border-purple-500/50 hover:shadow-2xl hover:shadow-purple-500/15 opacity-0 animate-fade-in-up delay-150 group overflow-hidden">
        ...
        <div className="relative">
          ...
          <ul className="space-y-2 sm:space-y-3 text-sm">
            ...
            <li className="flex flex-col md:flex-row md:items-center gap-1 md:gap-2"><span className="text-gray-400 hidden md:inline">•</span><ExternalLink href="https://github.com/minima-global" className="text-blue-400 hover:text-purple-400 underline decoration-blue-500/30 hover:decoration-purple-500/60 underline-offset-4 transition-all duration-300 font-medium break-all min-w-0">github.com/minima-global</ExternalLink><span className="text-gray-500 hidden md:inline">—</span><span className="text-gray-500 text-xs md:text-sm">Official GitHub</span></li>
          </ul>
        </div>
      </div>
      ...
      <article className="prose prose-invert max-w-none">
          ...
          <div className="relative bg-gray-800/40 border border-gray-700/40 rounded-2xl p-4 sm:p-6 transition-all duration-300 hover:border-purple-500/50 hover:shadow-2xl hover:shadow-purple-500/15 group overflow-hidden">
            ...
            <div className="mt-4 pt-3 border-t border-gray-700/40">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                <StatusBadge status="confirmed" /><span className="text-gray-500 hidden sm:inline">|</span><StatusBadge status="in-development" /><span className="text-gray-500 hidden sm:inline">|</span><StatusBadge status="community" />
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="border-t border-gray-700/40 pt-6 opacity-0 animate-fade-in-up delay-300">
          <p className="text-gray-500 text-sm flex items-center gap-2"><span className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />Last updated: 11 июня 26</p>
        </section>
      </article>
    </div>
  );
